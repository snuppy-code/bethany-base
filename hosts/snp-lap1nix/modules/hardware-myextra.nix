{
  ...
  # Nya thinks the problem is with d3cold, so this just matches for some of the wifi card's attributes that nya found with udevadm and lspci, and sets d3cold_allowed to "0"
  services.udev.extraRules = ''
    ACTION=="add", SUBSYSTEM=="pci", ATTR{vendor}=="0x8086", ATTR{subsystem_vendor}=="0x8086", ATTR{subsystem_device}=="0x00f4", ATTR{device}=="0x272b", ATTR{d3cold_allowed}="0"
  '';
}
